...
contract VaultTokenFactory is IVaultTokenFactory {
	...
	event VaultTokenCreated(uint256 indexed pid, address vaultToken, uint vaultTokenIndex);

	constructor(address _router, address _masterChef, address _rewardsToken, uint256 _swapFeeFactor) public {
		require(_swapFeeFactor >= 900 && _swapFeeFactor <= 1000, "VaultTokenFactory: INVALID_FEE_FACTOR");
		router = _router;
	...
	function allVaultTokensLength() external view returns (uint) {
		return allVaultTokens.length;
	}

	function createVaultToken(uint256 pid) external returns (address vaultToken) {
		require(getVaultToken[pid] == address(0), "VaultTokenFactory: PID_EXISTS");
		bytes memory bytecode = type(VaultToken).creationCode;
		assembly {
			vaultToken := create2(0, add(bytecode, 32), mload(bytecode), pid)
		}
		VaultToken(vaultToken)._initialize(IUniswapV2Router01(router), IMasterChef(masterChef), rewardsToken, swapFeeFactor, pid);
