...
contract SpiritToken is BEP20("SpiritSwap Token", "SPIRIT") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "SPIRIT::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "SPIRIT::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "SPIRIT::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "SPIRIT::_writeCheckpoint: block number exceeds 32 bits"
        );
...
contract SpiritMasterChef is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 spiritReward = multiplier
            .mul(spiritPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
            accSpiritPerShare = accSpiritPerShare.add(
                spiritReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accSpiritPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSpiritPerShare = pool.accSpiritPerShare.add(
            spiritReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accSpiritPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeSpiritTransfer(msg.sender, pending);
